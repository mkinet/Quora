...
def oversample_training_set(X,y):
    pos_train = X[y == 1]
    neg_train = X[y == 0]
...
def my_oversample_training_set(X,y,target):
    pos_train = X[y==1]
    neg_train = X[y==0]
    Npos = len(pos_train)

    Nneg = int(Npos*(1-target)/target)
    new_neg_train = neg_train.sample(Nneg,replace = True)
    
    X = pd.concat([pos_train, new_neg_train])
    y = np.hstack((np.zeros(len(pos_train)) + 1,np.zeros(len(new_neg_train))))
...
def main():
    ...
    X_train, X_valid, y_train, y_valid = train_test_split(X_train, y_train, test_size=0.1, random_state=3333)
    ...
    X_train, y_train = my_oversample_training_set(X_train,y_train,0.174)
    print(np.mean(y_train))
    ...
    X_valid,y_valid = my_oversample_training_set(X_valid,y_valid,0.174)
    ...
    sub.to_csv('model_from_K.csv')


def randomized_xgboost():
    X_train_full = pd.read_csv('x_train.csv',index_col =0)
    y_train_full = pd.read_csv('y_train.csv',index_col =0)    
    y_train_full = y_train_full['is_duplicate'].values
    x_test_full = pd.read_csv('x_test.csv',index_col =0)


    d_test = xgb.DMatrix(x_test_full)

    preds = pd.DataFrame(index=x_test_full.index)
    ...
    for i in range(30):
        print('Training model number %s' %str(i))
        X_train, X_valid, y_train, y_valid = train_test_split(X_train_full, 
                                                              y_train_full,
                                                              test_size=0.1)
        target = np.random.normal(0.17,5e-3)
        X_train, y_train = my_oversample_training_set(X_train,y_train,0.174)
        X_valid,y_valid = my_oversample_training_set(X_valid,y_valid,0.174)
        ...
        watchlist = [(d_train, 'train'), (d_valid, 'valid')]
        params['subsample'] = np.random.normal(0.62,1.5e-2)
        bst = xgb.train(params, d_train, 3000, watchlist, 
                        early_stopping_rounds=50, verbose_eval=50)
        print(log_loss(y_valid, bst.predict(d_valid)))
        bst.save_model( 'xgb.mdl')

        p_test = bst.predict(d_test)
        preds['pred_'+str(i)] = p_test
        print(preds.head())
        sub = preds.mean(axis =1)
        sub.to_csv('model10.csv')

def Lasso_pred():
    X_train = pd.read_csv('x_train.csv',index_col =0)
    y_train = pd.read_csv('y_train.csv',index_col =0)
    X_train.fillna(value=0.0, inplace=True)
    X_train.replace(np.Inf, value=0, inplace=True)
    ...
    for c in C:
        ...
        score, std = cv_score(clf, X_train, y_train)
        scores.append(score)
